...
>
> Maven, Java, Spring, API, component-dependency, and stable report analysis are available. The CLI serves each report to a packaged Vue application over a loopback-only endpoint. A responsive visual system and reusable product Shell are now in place; substantive product views are still under development, and the first release is not available yet.
