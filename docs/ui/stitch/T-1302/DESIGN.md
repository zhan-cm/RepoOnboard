...
# T-1302 — Settings Stitch Import

## Import

- Source: User-reviewed Stitch export
- Imported: 2026-09-20
- Screen: `screen.png`
- Reference markup: `code.html`
- Desktop viewport: 1600 × 992
- Provided variant: Light desktop

## Responsive and Theme

- Language choices remain a single vertical radio group at all widths.
- The local-preference cards use one column below the existing medium breakpoint and three columns above it.
- A separate narrow screenshot was not supplied; production must reuse the existing RepoOnboard narrow-shell behavior rather than introduce a drawer or alternate navigation system.
- A separate dark screenshot was not supplied; production must reuse the existing RepoOnboard theme tokens and current single-button theme toggle.

## Contract Alignment

- Settings is a utility destination separate from repository perspectives.
- The only language choices are `en` / English and `zh-CN` / 中文.
- Selection uses native radio inputs, a visible selected state, keyboard focus, and immediate feedback through an `aria-live="polite"` region.
- No Save, Apply, Reset, account, cloud sync, telemetry, language download, cache control, or report-dependent fact is part of the page.
- The page remains independent of `/api/report`, `/api/start-here`, Java APIs, and the public report schema.

## Reference-only Elements

- Tailwind CDN, Google Fonts, Material Symbols, inline JavaScript, and all utility classes in `code.html` are Stitch reference material only. They must not enter the production Vue/JAR resource closure.
- The generic design-system notes below are reference material, not additional T-1302 requirements. Items outside the page contract, including `Open in IDE` and `Jump to Symbol`, remain excluded.
- Production must reuse the repository's existing components, system-font stack, CSS tokens, preference adapters, and offline assets.

## Known Differences

- The prototype theme button communicates placement only; production retains the current light/dark label, icon, behavior, and persistence implementation.
- The prototype script demonstrates selected-state feedback only. T-1303 owns the validated `repoonboard-language` cookie/localStorage contract and actual localization state.
- No separate dark or narrow screenshot is included; both are implementation and browser-validation obligations under the existing Shell and T-1305.
